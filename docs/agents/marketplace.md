...
If `git rev-parse HEAD` is not the commit in the bot's latest validation
comment, edit that issue (same six headings, all five checkboxes checked).
Editing retriggers detection. A comment does not.
...
the pinned `install.sh` (URL + SHA-256 in `Panel.qml`); the script still
installs the current GitHub release tarball.

Done when the bot validation comment names current HEAD, the baseline is
`review-required` with capability `installer` only (or `passed`), and
...
`make bump` / `make pin-release`, push, and `make release` stay as in
`AGENTS.md`. Then file a **new** issue — not #4712.
...
If the bot fails, edit that Verify issue. A later `master` push without a
new Verify issue leaves the catalog `Update unverified`.
...
- Keep `install.sh` in `zerobearing2/omatalk`, not this tree.
- Re-pin when `install.sh` itself changes (`make pin-release`), not for
  every Daemon tarball.
